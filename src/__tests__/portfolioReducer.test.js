...
const item1 = { id: 'abc-1', src: '/img1.jpg', title: 'Work 1', category: 'Web Design' }
const item2 = { id: 'abc-2', src: '/img2.jpg', title: 'Work 2', category: 'Wordpress' }
...
describe('portfolioReducer', () => {
  ...
  test('CHANGE_LINK updates currLink', () => {
    const next = portfolioReducer(undefined, { type: 'CHANGE_LINK', payload: 3 })
  ...
  test('FETCH_PORTFOLIO_SUCCESS replaces portfolioData and preserves currLink', () => {
    ...
    const next = portfolioReducer(prev, { type: 'FETCH_PORTFOLIO_SUCCESS', payload: [item2] })
  ...
  test('CREATE_PORTFOLIO_SUCCESS appends item to empty list', () => {
    const next = portfolioReducer(undefined, { type: 'CREATE_PORTFOLIO_SUCCESS', payload: item1 })
  ...
  test('CREATE_PORTFOLIO_SUCCESS appends item to existing list', () => {
    ...
    const next = portfolioReducer(prev, { type: 'CREATE_PORTFOLIO_SUCCESS', payload: item2 })
  ...
  test('UPDATE_PORTFOLIO_SUCCESS replaces matching item by id', () => {
    ...
    const next = portfolioReducer(prev, { type: 'UPDATE_PORTFOLIO_SUCCESS', payload: updated })
  ...
  test('UPDATE_PORTFOLIO_SUCCESS leaves list unchanged when id not found', () => {
    const ghost = { id: 'ghost-id', src: '/x.jpg', title: 'Ghost', category: 'Web Design' }
    const prev = { currLink: 0, portfolioData: [item1] }
    const next = portfolioReducer(prev, { type: 'UPDATE_PORTFOLIO_SUCCESS', payload: ghost })
    expect(next.portfolioData).toEqual([item1])
  })

  test('DELETE_PORTFOLIO_SUCCESS removes item by string id', () => {
    const prev = { currLink: 0, portfolioData: [item1, item2] }
    const next = portfolioReducer(prev, { type: 'DELETE_PORTFOLIO_SUCCESS', payload: 'abc-1' })
    expect(next.portfolioData).toEqual([item2])
  })
  ...
  test('DELETE_PORTFOLIO_SUCCESS with unknown id leaves list unchanged', () => {
    const prev = { currLink: 0, portfolioData: [item1, item2] }
    const next = portfolioReducer(prev, { type: 'DELETE_PORTFOLIO_SUCCESS', payload: 'no-such-id' })
    expect(next.portfolioData).toEqual([item1, item2])
  })
  ...
  test('DELETE_PORTFOLIO_SUCCESS: numeric payload does not match string id (type-contract guard)', () => {
    const prev = { currLink: 0, portfolioData: [{ id: '1', src: '/a.jpg', title: 'A', category: 'Web Design' }] }
    const next = portfolioReducer(prev, { type: 'DELETE_PORTFOLIO_SUCCESS', payload: 1 })
